...
BEGIN
  ...
  DROP POLICY IF EXISTS "Organizations can view their subscription" ON subscriptions;
  DROP POLICY IF EXISTS "Admins can update their organization subscription" ON subscriptions;
  
  -- billing_details policies
  DROP POLICY IF EXISTS "Organizations can view their billing details" ON billing_details;
  DROP POLICY IF EXISTS "Admins can manage billing details" ON billing_details;
  
  -- invoices policies
  DROP POLICY IF EXISTS "Organizations can view their invoices" ON invoices;
  
  -- usage_logs policies
  DROP POLICY IF EXISTS "Organizations can view their usage" ON usage_logs;
END $$;
...
CREATE POLICY "Organizations can view their subscription" ON subscriptions
  FOR SELECT
  USING (
    EXISTS (
...
CREATE POLICY "Organizations can view their billing details" ON billing_details
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM organization_members
...
CREATE POLICY "Admins can manage billing details" ON billing_details
  FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM organization_members
...
CREATE POLICY "Organizations can view their usage" ON usage_logs
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM organization_members
